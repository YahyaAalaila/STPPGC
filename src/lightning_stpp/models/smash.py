...
from smash.model import Transformer_ST, ScoreMatch_module, SMASH, Model_all
# --------------------------------------------------------------------
@BaseSTPPModule.register("smash")
class SMASHSTPP(BaseSTPPModule):
    """
    Thin Lightning wrapper around the original SMASH implementation.
    ...
    def __init__(self, model_cfg: Config, data_cfg: DataConfig):
        ...
        core = self._score_match(model_cfg)
        self.smash = self._smash(core, model_cfg)

        self.model: Model_all = Model_all(self.transformer, self.smash)
    ...
    def forward(self, st_x: Tensor, mask: Tensor) -> Any:
        ...
        return self.smash(st_x, enc_out)
    ...
    def _shared_step(self, batch, stage: str):
        ...
        loss = self.smash(seq_non, enc_out_non_mask)
    ...
    @staticmethod
    def _score_match(model_cfg):
        return ScoreMatch_module(
            dim        = 1 + model_cfg.dim,
            condition  = True,
    ...
    @staticmethod
    def _smash(core, model_cfg):
        return SMASH(
            core,
            sigma              = (model_cfg.sigma_time, model_cfg.sigma_loc),
            seq_length         = 1 + model_cfg.dim,
            sampling_timesteps = model_cfg.samplingsteps,
            n_samples          = model_cfg.n_samples,
            langevin_step      = model_cfg.langevin_step,
            num_types          = model_cfg.num_types,
            loss_lambda        = model_cfg.loss_lambda,
            loss_lambda2       = model_cfg.loss_lambda2,
            smooth             = model_cfg.smooth,
        )
